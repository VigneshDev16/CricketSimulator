...
const Home: () => JSX.Element = () => {
  ...
  const OnBowlClicked = async () => {
    ...
    if (striker) {
      value = await generateWeightedRandom(
        ScoreInfo.scoreInfo,
        striker.probability,
      );
    }
  ...
  async function generateWeightedRandom(items: number[], weights: number[]) {
    return RTNWeightedRandNum?.weightedRandNum(items, weights);
  }
